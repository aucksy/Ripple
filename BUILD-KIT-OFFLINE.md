...
| 1 | Settings, and the published-tables list | `ripple/config.py`, `ripple/production.py`, `tests/test_production.py` | 350 lines |
...
| 8 | Progress, saved history, and the web service | `ripple/progress.py`, `ripple/store.py`, `ripple/build_info.py`, `ripple/api.py`, `run.py`, `tests/test_api.py` | 1,000 lines |
...
trouble is that the other windows are building against it and none of them
...
source_kind. Read three shapes:
  ...
  .msg   Outlook's compound file format. On this machine there is no package
         that opens it and none can be installed, so Ripple does not read .msg
         files here and must say so plainly rather than half-read one.
         A partial read is the dangerous outcome: a reader that finds the
         subject stream but not the body leaves the body blank, a blank body
         matches no table in the catalogue, and the confirm screen then shows an
         empty form as though the email said nothing.
         So when a .msg is uploaded, come back with the warning "Ripple cannot
         read Outlook .msg files on this machine." and name the two ways in that
         this screen actually has: save the email as plain text or .eml from
         Outlook and upload that, or use the manual tab and type the tables and
         attributes. Do not point at a paste box - there is none, and a dead end
         dressed as an instruction is worse than a plain refusal.
         Test it: a .msg upload comes back with that warning and with the two
         alternatives named, never as an empty email.
...
`ripple-build/ripple/build_info.py`, `ripple-build/ripple/api.py`,
`ripple-build/tests/test_api.py`, and `ripple-build/run.py` at the project root
...
Build ripple/progress.py, ripple/store.py, ripple/build_info.py,
ripple/api.py and tests/test_api.py.
...
RIPPLE writes a version. The line on the settings screen reads it from here, and so
does anything you name a copy of the folder after when you hand it to somebody.
...
DO NOT FIX THE PORT AT 8000. Take the first free port between 8000 and 8020:
bind a socket to each in turn and keep the first that binds. A fixed port is
fine until the day something else on the machine already has it, and then the
program stops with a Windows socket error that names no application and tells
nobody what to close — on the one machine where nobody can go and look. If all
twenty-one are taken, say exactly that in one sentence: which ports were tried,
and that whatever is using them has to be closed first.

Print the address you actually got, not the one you asked for.
...
17. **The settings screen names this build.** It shows a version and a date.
    Nothing on this machine packages Ripple and there is usually no git here,
    so the line should end "no build record — this is a guess", with a note
    underneath saying the date is the newest file in this folder. That is the
    correct answer here, not a fault: nothing has stamped this copy, and a date
    that looked like a fact would be worse than the honest line.

    If you want a real date on a copy you are handing to somebody else, stamp
    it yourself before you hand it over. From the project root:

    ```
    python -c "from pathlib import Path; from ripple.build_info import write_stamp; print(write_stamp(Path('.')))"
    ```

    That writes BUILD-STAMP.json beside run.py, and the screen reads "built"
    and that date from then on. Run it again each time you hand the folder on;
    an old stamp is a fact about the wrong day.

    Write ripple/build_info.py exactly as the other kit has it, including the
    look inside a packaged folder that this kit never produces. It costs
    nothing, and it keeps one file rather than two that drift apart.
