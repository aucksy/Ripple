...
def build_kit(out_path: Path, title: str, intro: str, files, check_name: str,
    ...
    for (saved_as, npieces, nlines, _), (_, _, _, what) in zip(plan, files):
        parts.append(f"| `{saved_as}` | {what} | {nlines:,} | {npieces} |")
    parts.append(f"""
...
""")
    ...
    out_path.write_text("\n".join(parts) + "\n", encoding="utf-8")
    return total_pastes, total_lines
...
def main() -> None:
    ...
    print(f"BUILD-KIT-UI-EXACT.md      {n1:>3} pastes  {l1:>7,} lines")

    n2, l2 = build_kit(ROOT / "BUILD-KIT-ENGINE-EXACT.md", "engine", ENGINE_INTRO,
                       engine_files(), "check_engine.py", ENGINE_TAIL)
    print(f"BUILD-KIT-ENGINE-EXACT.md  {n2:>3} pastes  {l2:>7,} lines")
    print(f"\ntogether: {n1 + n2} pastes, {l1 + l2:,} lines — plus sqlglot, which must be copied")
